...
def getResultSequences(data):
    ...
    while len(availableNods) > 0:
        currentNods = []
        i = 0
        while i < WORKERS_COUNT and len(availableNods) > 0:
            currentNod = availableNods.pop()
            currentNods.append(currentNod)

        resultTime += ord(max(currentNods))-DELTA_TO_CALC_SECONDS

        availableNods.extend(getNextNods(currentNods, mapOfPrevious))
        availableNods = sorted(availableNods, reverse=True)
