...
def if_exist_delete_combination(big, small, cur_unit_index, input_list):
    if big - UP_LOW_DEFFERENCE == small:
        input_list.pop(cur_unit_index)
        input_list.pop(cur_unit_index)
        if cur_unit_index > 0:
...
def getResultSeq(input_list):
    ...
    while True:
        ...
        for i in range(input_length):
            ...
            current = ord(input_list[i])
            next = ord(input_list[i+1])

            if current > next:
                i = if_exist_delete_combination(
                    current, next, i, input_list)
            else:
                i = if_exist_delete_combination(
