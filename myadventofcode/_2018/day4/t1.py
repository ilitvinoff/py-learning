...
def getResult(data) -> str:
    ...
    for guard_from_dict, minutes_from_dict in guard_dict.items():
        ...
        minutes_sum = sum(minutes_from_dict.values())
        print('guard:', guard_from_dict, '; slept:', minutes_sum,
              '; most common:', minutes_from_dict.most_common(1))
        if minutes_sum > max_most_common:
            guard_id = guard_from_dict
            max_most_common = minutes_sum
            num_minute = minutes_from_dict.most_common(1)

    print(guard_id)
    print((num_minute[0][0]))
    print(int(guard_id)*num_minute[0][0])
